...
import PcBrand from "../../../models/electronics/pc/pc_brand_model.js";
import {uploadPcBrandImage,deletePcBrandImage} from "../../../utils/electronics/pc/pcBrand.js";
export const addPcBrand=async(req,res)=>{
  try{
    ...
    const existing=await PcBrand.findOne({name:new RegExp(`^${name.trim()}$`,"i")});
    ...
    const brand=await PcBrand.create({name:name.trim(),logoUrl});
...
export const deletePcBrand=async(req,res)=>{
  try{
    const brand=await PcBrand.findById(req.params.id);
    if(!brand)return res.status(404).json({success:false,message:"Brand not found"});
    await deletePcBrandImage(brand.logoUrl);
