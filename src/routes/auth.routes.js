...
// ==================================================
// VALID LANGUAGE CODES (must match schema enum)
// ==================================================
...
// ==================================================
// PHONE — STRING ONLY — EXACTLY 10 DIGITS
// ==================================================
...
router.post("/register", async (req, res) => {
  try {
    ...
    if (finalEmail) {
      orConditions.push({ email: finalEmail });
    }
    ...
    const userData = {
      ...
      language: finalLanguage, // ✅ "ta", "en" etc.
      address: address || "NA",
    };
...
router.post("/forgot-request", async (req, res) => {
  try {
    ...
    console.log("FORGOT REQUEST PHONE:", phone);

    const user = await User.findOne({ phone: phone });

    if (!user) {
      console.log("FORGOT REQUEST USER NOT FOUND:", phone);
    ...
    user.forgotRequestAt = new Date();
    user.requestedPassword = newPassword;

    await user.save();

    console.log("FORGOT REQUEST SAVED:", user._id.toString());
