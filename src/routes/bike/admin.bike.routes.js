...
const prepareSellerForResponse=bike=>{
  if(!bike)return bike;
  if(typeof bike.seller==="string"&&bike.seller.includes(":"))
    try{bike.seller=decryptSeller(bike.seller)}catch(_){}
  return bike;
};
const getRefId=x=>x?._id||x;
const attachAdminUsers=async bikes=>{
  const list=Array.isArray(bikes)?bikes:[bikes];
  const ids=[...new Set(list.flatMap(b=>[
    getRefId(b?.createdBy),
    getRefId(b?.sellerUser)
  ]).filter(x=>x&&mongoose.Types.ObjectId.isValid(x.toString())).map(x=>x.toString()))];
  if(!ids.length)return bikes;
  const users=await User.find({_id:{$in:ids}}).select(ADMIN_USER_FIELDS).lean();
  const map=new Map(users.map(u=>[u._id.toString(),u]));
  list.forEach(b=>{
    const createdId=getRefId(b?.createdBy);
    const sellerId=getRefId(b?.sellerUser);
    if(createdId){
      const u=map.get(createdId.toString());
      if(u)b.createdBy=u;
    }
    if(sellerId){
      const u=map.get(sellerId.toString());
      if(u)b.sellerUser=u;
    }
  });
  return bikes;
...
const validateRegistration=(state,number)=>{
  const registrationState=String(state||"TN").trim().toUpperCase();
  ...
  if(!/^[0-9]{2}$/.test(registrationNumber))
    throw new Error("Registration number must contain exactly 2 digits");
...
const validateBikeHierarchy=async(brand,model,variant=null)=>{
  ...
  if(modelDoc.brand&&modelDoc.brand.toString()!==brand.toString())
  ...
  if(variant){
    ...
    if(variantDoc.model&&variantDoc.model.toString()!==model.toString())
      throw new Error("Selected variant does not belong to selected model");
  }
};
const uploadBikeMedia=async files=>{
  const bannerImage=files?.banner?.length
    ?await uploadBikeImage(files.banner[0],"bikes/banner"):null;
  const galleryImages=files?.gallery?.length
    ?await Promise.all(files.gallery.map(file=>uploadBikeImage(file,"bikes/gallery"))):[];
  const audioNote=files?.audio?.length
    ?await uploadBikeImage(files.audio[0],"bikes/audio"):null;
  const videoFiles=[...(files?.video||[]),...(files?.videos||[])];
  const videos=videoFiles.length
    ?await Promise.all(videoFiles.map(file=>uploadBikeImage(file,"bikes/videos"))):[];
...
const safeDeleteMedia=async media=>{
  ...
  try{await deleteBikeImage(media)}
  catch(error){console.error("BIKE MEDIA DELETE ERROR:",error.message)}
};
const uploadFields=[
...
router.get("/user-by-phone",verifyToken,isAdmin,async(req,res)=>{
  try{
    ...
    console.error("ADMIN BIKE USER PHONE SEARCH ERROR:",error);
...
router.post("/add",verifyToken,isAdmin,uploadBike.fields(uploadFields),async(req,res)=>{
  try{
    const{
      brand,model,variant,registrationState,registrationNumber,
      year,price,km,seller,sellerUser,sellerinfo,district,city,
      description,insurance,status,videoLink
    }=req.body;
    ...
    const finalSeller=
      seller&&typeof seller==="string"&&seller.trim()
        ?encryptSeller(seller.trim()):null;
    const bike=await Bike.create({
      ...
      km:km===""||km===undefined?null:km,
      seller:finalSeller,
      sellerUser:sellerUser||null,
      sellerinfo:sellerinfo||"Rc owner",
      district,
      city:city||null,
    ...
    return res.status(201).json({
      ...
      bike:prepareSellerForResponse(bike.toObject())
    });
  }catch(error){
    console.error("ADMIN ADD BIKE ERROR:",error);
    return res.status(400).json({
      success:false,
      message:error.message||"Failed to add bike"
    });
  }
});
router.get("/all",verifyToken,isAdmin,async(req,res)=>{
  try{
    const{
      brand,model,variant,status,district,city,insurance,search,
      minPrice,maxPrice,minYear,maxYear,registrationState
    }=req.query;
    const filter={};
    ...
    if(registrationState)
      filter.registrationState=String(registrationState).trim().toUpperCase();
    if(search){
      filter.$or=[
    ...
    return res.status(500).json({
      success:false,
      message:error.message||"Failed to get all bikes"
    });
  }
});
router.get("/:id",verifyToken,isAdmin,async(req,res)=>{
  try{
    ...
    return res.json({
      success:true,
      bike:prepareSellerForResponse(bike)
    });
  }catch(error){
    console.error("ADMIN GET BIKE ERROR:",error);
    return res.status(500).json({
      success:false,
      message:error.message||"Failed to get bike"
    });
  }
});
router.patch("/:id/status",verifyToken,isAdmin,async(req,res)=>{
  try{
    ...
    return res.json({
      ...
      bike:prepareSellerForResponse(responseBike)
    });
  }catch(error){
    console.error("ADMIN BIKE STATUS ERROR:",error);
    return res.status(400).json({
      success:false,
      message:error.message||"Failed to update bike status"
    });
  }
});
router.put("/:id",verifyToken,isAdmin,uploadBike.fields(uploadFields),async(req,res)=>{
  try{
    ...
    if(!bike)
      return res.status(404).json({success:false,message:"Bike not found"});
    const finalBrand=req.body.brand!==undefined
      ?req.body.brand:bike.brand;
    const finalModel=req.body.model!==undefined
      ?req.body.model||null:bike.model;
    const finalVariant=req.body.variant!==undefined
      ?req.body.variant||null:bike.variant;
    ...
    const registration=validateRegistration(
      req.body.registrationState!==undefined
        ?req.body.registrationState:bike.registrationState,
      req.body.registrationNumber!==undefined
        ?req.body.registrationNumber:bike.registrationNumber
    );
    const allowedFields=[
      "year",
      "price",
    ...
    if(req.body.seller!==undefined){
      bike.seller=
        req.body.seller&&String(req.body.seller).trim()
          ?encryptSeller(String(req.body.seller).trim())
          :null;
    }
    ...
    if(req.files?.gallery?.length){
      if(Array.isArray(bike.galleryImages))
        for(const oldImage of bike.galleryImages)
          await safeDeleteMedia(oldImage);
      bike.galleryImages=await Promise.all(
        req.files.gallery.map(image=>
          uploadBikeImage(image,"bikes/gallery")
        )
      );
    }
    ...
    if(videoFiles.length){
      if(Array.isArray(bike.videos))
        for(const oldVideo of bike.videos)
          await safeDeleteMedia(oldVideo);
      bike.videos=await Promise.all(
        videoFiles.map(video=>
          uploadBikeImage(video,"bikes/videos")
        )
      );
    }
    await bike.save();
...
router.delete("/:id",verifyToken,isAdmin,async(req,res)=>{
  try{
    ...
    await safeDeleteMedia(bike.bannerImage);
    if(Array.isArray(bike.galleryImages))
      for(const image of bike.galleryImages)
        await safeDeleteMedia(image);
    await safeDeleteMedia(bike.audioNote);
    if(Array.isArray(bike.videos))
      for(const video of bike.videos)
        await safeDeleteMedia(video);
...
router.patch("/:id/approve",verifyToken,isAdmin,async(req,res)=>{
  try{
    ...
    return res.status(500).json({
      success:false,
      message:error.message||"Failed to approve bike"
    });
  }
});
router.patch("/:id/reject",verifyToken,isAdmin,async(req,res)=>{
  try{
    ...
    return res.json({
      success:true,
      message:"Bike moved to draft successfully",
      bike:prepareSellerForResponse(responseBike)
    });
  }catch(error){
    ...
    return res.status(500).json({
      success:false,
      message:error.message||"Failed to reject bike"
    });
  }
});
