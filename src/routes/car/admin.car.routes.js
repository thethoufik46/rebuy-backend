...
router.put("/:id/request-delete", verifyToken, async (req, res) => {
  try {
    ...
    const isOwner =
      car.createdBy &&
      car.createdBy.toString() === req.user.id.toString();
    const isSeller =
      car.sellerUser &&
      car.sellerUser.toString() === req.user.id.toString();
    if (!isOwner && !isSeller)
    ...
    return res.json({
      success: true,
      message: "Car delete request submitted",
      carId: car._id,
      status: car.status
    });
  } catch (error) {
    ...
    return res.status(400).json({
      success: false,
      message:
        error.message || "Failed to request car deletion"
    });
  }
});
