...
const bannerImage=req.files?.banner?await uploadElectronicsMedia(req.files.banner[0],"electronics/banner"):null;
...
const finalElectronics=electronics.map(item=>{if(typeof item.seller==="string"&&item.seller.includes(":"))try{item.seller=decryptSeller(item.seller)}catch(_){}return item});
res.json({success:true,count:finalElectronics.length,electronics:finalElectronics});
}catch(err){console.log("ADMIN GET ELECTRONICS ERROR:",err);res.status(500).json({success:false,message:"Failed to fetch electronics"});}
});
...
let gallery;try{gallery=Array.isArray(req.body.existingGallery)?req.body.existingGallery:JSON.parse(req.body.existingGallery)}catch{gallery=item.galleryImages||[]}
if(Array.isArray(gallery)){for(const x of(item.galleryImages||[]).filter(x=>!gallery.includes(x)))await deleteElectronicsMedia(x);item.galleryImages=gallery;}
}
...
let videos;try{videos=Array.isArray(req.body.existingVideos)?req.body.existingVideos:JSON.parse(req.body.existingVideos)}catch{videos=item.videos||[]}
if(Array.isArray(videos)){for(const x of(item.videos||[]).filter(x=>!videos.includes(x)))await deleteElectronicsMedia(x);item.videos=videos;}
}
