...
const fields=[{name:"banner",maxCount:1},{name:"gallery",maxCount:10},{name:"audio",maxCount:1},{name:"video",maxCount:3}];
router.get("/",verifyTokenOptional,async(req,res)=>{
  try{
    const isAdmin=req.user?.role==="admin",query={}, {category,brand,district,minPrice,maxPrice}=req.query;
...
router.post("/user-add",verifyToken,uploadElectronics.fields(fields),async(req,res)=>{
  try{
    ...
    const item=await Electronics.create({...req.body,bannerImage,galleryImages,audioNote,videos,videoLink:videoLink||null,seller:String(user.phone),sellerUser:user._id,createdBy:user._id,status:"draft",price:null});
    res.status(201).json({success:true,message:"Electronics submitted for approval",item});
  }catch(err){console.log("USER ADD ERROR:",err);res.status(500).json({success:false,message:err.message});}
});
...
router.put("/:id/request-delete",verifyToken,async(req,res)=>{
  try{
    ...
    if(item.createdBy.toString()!==req.user.id.toString())return res.status(403).json({success:false,message:"Unauthorized"});
    item.status="delete_requested";
    await item.save();
    res.json({success:true,message:"Delete request sent"});
  }catch(err){res.status(500).json({success:false,message:"Request failed"});}
});
